import os
import logging
from config.config import UPLOAD_FOLDER, RESULT_FOLDER, SECRET_KEY
...
from converter.Validation import Validation
from converter.VideoConverter import VideoConverter
from helper.VideoHelper import VideoHelper
from converter.VideoUploader import VideoUploader
...
def upload():
    ...
    if request.method == 'POST':
        ...
        daily_folder = video_helper.generate_daily_folder(UPLOAD_FOLDER)
        out_path = video_helper.generate_upload_pathname(daily_folder)
        uploader = VideoUploader(received_file, out_path)
        if uploader.save_file():
